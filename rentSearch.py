...
class RentSearch:
    def __init__(self):
        ...
        def fetch():
            ...
            try:
                ...
                database.myQuery.execute(
                    f"select * from myrent where r_id={yup}")
                if database.myQuery:
                    myRow = database.myQuery.fetchone()
                    if myRow is not None:
                        frame.destroy()
                        rentSearhResult.RentSearchResult(myRow)
                    else:
                        popup.showwarning(
        ...
        search = Label(headerLabel, text="Enter Rent's ID : ",
        ...
        searchButton.place(x=780, y=16, width=50, height=27)
